# Overwatch - Data Models - V1

Reviewed: Jan-25-2026

## What?

**Project Overwatch** is an **Enterprise PMO & Governance Suite** – a centralized platform for strategic project oversight. The application tracks projects across their full lifecycle, managing governance, execution, control registers, and knowledge assets. It enforces standardized measurement and reporting structures across the portfolio.

**V1 Philosophy:** This system prioritizes **human judgment over automation**. It does not attempt to derive the project status algorithmically; it asks the Project Manager to declare it. The system serves as the single source of truth for *what was decided*, *what is at risk*, and *where we stand*.
...
- **Purpose:** Universal comment system providing a chronological, immutable audit trail of remarks, context, and decisions across all entities
- **Fields:**
  - id (PK)
...
- rag_status (RagStatus Enum) - **Manually set by PM** - Overall health indicator
- start_date
- target_end_date
- actual_end_date
- project_manager_id (FK to users)
- executive_sponsor_id (FK to users)
- deleted_at (Laravel soft delete)
- metadata (created_at, created_by, updated_at, updated_by)

#### Queries/Processes
- CRUD operations
- **Manual RAG Status:** PM selects Red/Amber/Green from a dropdown based on their professional judgment
...
- PM must justify RAG status selection in the Executive Summary
...
- is_critical_path (Boolean) - **Manual flag set by PM** for visual highlighting and filtering
- completed_at
...
- Special query: Non-completed/cancelled items from past + 4 weeks ahead (ISO 8601)

#### Business Rules
- **Grouping Hierarchy:** Maximum 2 levels (Parent Group → Child Group → Items)
- The system enforces that a group with a parent_id cannot itself be a parent
- **Critical Path Flag:**
  - PM manually checks/unchecks the "Critical Path" checkbox
  - **Visual indicator only** - highlights these milestones in UI (bold, color coding, badges)
  - Used for filtering: "Show me only critical path milestones."
  - **No automation** - does not trigger alerts, does not auto-calculate project delays, does not affect RAG status
  - PM uses this to communicate "these are the make-or-break dates" to stakeholders
...
#### Action Item Groups (aitem_groups)
- id (PK)
- project_id (FK)
- title
...
- Impact and urgency can be tracked in notes, but they do not auto-compute priority
...
- description (markdown)
- assigned_to (FK to users) - nullable
- deliverable_type (DeliverableType Enum)
- target_date
- target_week (ISO 8601) - **Auto-computed from target_date**
- status (ExecutionStatus Enum)
- priority (low, medium, high, critical) - **Manually set by PM**
...
#### Deliverable Groups (deliverable_groups)
- id (PK)
- project_id (FK)
- title
- parent_id (FK) - maximum 2 levels
...
- Assign/reassign/de-assign to groups
- Auto-compute target_week
- Query filters: status, is_internal_only
- Special query: Non-completed/cancelled items from past + 2 weeks ahead (ISO 8601)
...
- probability (RiskProbability Enum) - 1-5 scale
- impact (RiskImpact Enum) - 1-5 scale
...
- **No Auto-Risk Score:** PM assesses severity based on the probability and impact fields, but makes the final determination manually
...
- id (PK)
- project_id (FK)
- issue_group_id (FK) - nullable
- title
- description (markdown)
- issue_category (scope, schedule, resource, quality, communication, technical, other)
- impact (IssueImpact Enum)
- urgency (IssueUrgency Enum)
- priority (IssuePriority Enum) - **Manually set by PM**
...
- Once approved, PM manually updates project baseline dates, milestones, or deliverables to reflect the change
...
- status (DecisionStatus Enum) - Tracks decision lifecycle
...
#### Queries/Processes
- CRUD operations
- Display chronologically on the project dashboard
- Query filters: entry_type, entry_date, is_internal_only
- Special query: Recent entries (last 2 weeks or 4 most recent)

#### Cross-Module Linking
- Journal entries can link to domain items (Milestones, Deliverables, Action Items, Dependencies, Meetings, Risks, Issues, Change Requests, Decisions) using a simplified linking mechanism
- **Implementation:** Via the `entity_links` table (limited use):
...
- agenda (markdown)
- raw_minutes (markdown)
- processed_minutes (markdown)
...
- id (PK)
- project_id (FK)
- stakeholder_group_id (FK) - nullable
- name
- title
- organization
- role_in_project (StakeholderRole)
- contact_email
- contact_phone
- influence (Priority Enum)
- interest (Priority Enum)
...
- CRUD for stakeholders and groups
...
#### Queries/Processes
- CRUD operations
- Export lessons for the organizational knowledge base
...
- deleted_at (Laravel soft delete)
- is_active (Boolean) (Can use the software or not)
- last_login_at
...
- **"Stale Report" Indicator:** Flag projects without a Status Report published in >30 days
...
- All entities use the `deleted_at` flag
...
### Audit Trail
- Use `spatie/laravel-activitylog` or a custom audit table
...
- The system enforces a maximum 2-level hierarchy
...
- **Business Logic:**
  - Once approved, PM manually updates project baselines, milestones, or deliverables to reflect the change
  - Target week auto-calculated on target date change
  - **Manual RAG status:** PM selects from the dropdown
  - **Manual priority:** PM sets based on context (no auto-calculation)
  - **Manual risk assessment:** No auto-scoring
  - Grouping hierarchy is limited to a maximum of 2 levels

- **Visibility Control:**
  - The `is_internal_only` flag controls item visibility based on user context
  - Filtering is applied in all queries

---

## Target Week Display Rule

Transform the stored ISO 8601 Week (2026-W04) into a Week-of-Month format (Week 4 of January). Use this rule:

### The "Parse-then-Format" Rule

1. Parse: Use setISODate() to turn your stored string into a Carbon instance.
2. Extract: Use the weekOfMonth property for the week number.
3. Display: Use format('F') for the month name.

Code Implementation:
```php
$storedValue = "2026-W04"; // Your DB value
[$year, $week] = explode('-W', $storedValue);

$date = Carbon::now()->setISODate($year, $week);

return "Week {$date->weekOfMonth} of {$date->format('F')}";
// Result: "Week 4 of January"
```

Why this works: It treats the Monday of your stored ISO week as the anchor point for the monthly calculation.
...
6. **Selective Visibility:** The `is_internal_only` flag enables appropriate information sharing
