...
> - **Decisions:** Keep **critical items** (capture criticality)
> - **No linking** across modules for now

#### 1) Risks
- Track future uncertain events.
- **No auto-score:** the system does not compute Probability × Impact.
- **Minimum fields (recommended V1):** Title, Description (optional), Category (optional), Probability (optional), Impact (optional), **Severity (manual)**, Owner (optional), Status, Notes (optional), Attachments/Links (optional)

#### 2) Issues
- Track active problems (current-state blockers).
- **Status:** pending | on_track | at_risk | delayed | blocked | completed | cancelled
- Keep fields minimal and aligned with Action Items (Title, Target Date, Owner optional, Status, Notes, Attachments/Links).

#### 3) Change Requests (Scope + Schedule Only)
- Financial impacts are **out** for V1.
- Capture:
  - **Scope Impact (structured):** increase / decrease / none + description
  - **Schedule Impact (structured):** days/weeks/months (numeric + unit) + description
- Keep a simple workflow: Draft → Approved → Rejected (with mandatory rationale).

#### 4) Decisions (Critical Log)
- Use the Decisions log for **critical decisions** (not every minor choice).
- **Minimum fields:** Title, Decision, Rationale, Decision Maker (optional), Decision Date, **Critical (yes/no)**, Notes (optional), Attachments/Links (optional)
- Keep the record immutable in practice (edit window can be set later), but in V1 treat it as “write carefully.”
