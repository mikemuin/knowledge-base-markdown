...
**Read this in conjunction with:** `AI_Agent_Instructions_Folder_Structure_V2.md`

### Key Corrections in V2

This version corrects the following critical issues from V1:

1. **Soft Delete Implementation:** Now uses Laravel's standard `SoftDeletes` trait with nullable timestamp (not boolean)
2. **Meeting Event Status:** Separated from ExecutionStatus with proper enum values
3. **Complete Enum Coverage:** Added all missing enums (MeetingEventStatus, MeetingType, DeliverableType, DependencyType, DependencyStatus, DecisionStatus, and all LessonsLearned enums)
...
**Snapshot Trigger:** When `published_at` is set, capture snapshot data:
...
// Allow soft-delete (sets deleted_at timestamp)
...
**Superseding Logic:** When creating a new decision that supersedes an old one:
...
**Mechanism:** Use Laravel's standard `SoftDeletes` trait with nullable timestamp.

**Implementation:**

```php
// Migration
$table->softDeletes(); // Creates nullable `deleted_at` timestamp
...
// Query only non-deleted records (automatic via global scope)
Milestone::all();

// Query including soft-deleted records
Milestone::withTrashed()->get();

// Query only soft-deleted records
Milestone::onlyTrashed()->get();
...
### Execution Entities (Milestones, Action Items, Deliverables)
...
### Meeting Event Status

```php
enum MeetingEventStatus: string
{
...
{
    case STEERING_COMMITTEE = 'steering_committee';
    case WORKING_GROUP = 'working_group';
    case STATUS_REVIEW = 'status_review';
    case OTHER = 'other';
}
```
...
```php
enum LessonsLearnedPhase: string
{
...
```php
enum LessonsLearnedCategory: string
{
...
```php
enum LessonsLearnedImportance: string
{
    case LOW = 'low';
    case MEDIUM = 'medium';
    case HIGH = 'high';
...
**Rendering:** Use a markdown parser (e.g., `league/commonmark`):
...
**Primary:** MySQL (production-grade, robust) **Local:** SQLite (high-performance local development)
...
3. **Use Laravel's standard `SoftDeletes` trait** with nullable timestamp
