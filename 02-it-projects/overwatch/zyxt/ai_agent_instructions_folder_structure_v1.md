...
This codebase follows a **modular monolith** approach with **flat module separation**. Each business feature is represented as a self-contained module at the root level.

**The Four Core Domains (Mental Model Only):**
...
**Note:** Domains are conceptual groupings only. The actual folder structure is flat—all modules live directly under `app/Modules/`.
...
Each module is a self-contained feature with isolated structure.
...
**Module List by Domain (Conceptual Grouping):**
...
```
app/Modules/ActionItems/Services/ActionItemService.php
app/Modules/Risks/Repositories/RiskRepository.php
app/Modules/ProjectJournal/Events/JournalEntryCreated.php
```
...
Many modules support **2-level hierarchical grouping**. These require companion models:
...
- **Implementation:**
  ...
  ```
  app/Modules/Shared/Notes/├── Models/Note.php├── Services/NoteService.php├── Policies/NotePolicy.php└── Resources/NoteResource.php
  ```
...
- **Implementation:**
  ...
  ```
  app/Modules/Shared/EntityLinks/├── Models/EntityLink.php└── Services/EntityLinkService.php
  ```
...
1. **Keep each module self-contained**. No logic leakage across modules except via explicit Service contracts.
...
6. **Soft deletes only** — No hard deletes allowed.
   - Use `deleted_at` timestamp (nullable)
   - Implement Laravel's `SoftDeletes` trait on all domain models
...
app/Modules/ActionItems/FormRequests/Api/StoreActionItemRequest.php

// DTO (Domain Layer - Reusable)
app/Modules/ActionItems/DTOs/CreateActionItemData.php
...
- **Implementation:**
  ...
  ```php
  // In Model Observer or Service$entity->target_week = Carbon::parse($entity->target_date)->format('o-\WW');// In ViewModel/Resource for display[$year, $week] = explode('-W', $entity->target_week);$date = Carbon::now()->setISODate($year, $week);return "Week {$date->weekOfMonth} of {$date->format('F')}";
  ```
...
- Services calling other Services across modules (via dependency injection)
- Services emitting Events consumed by other modules
...
- Controllers directly accessing other Module Services
- Bypass of Policy authorization checks
- Cross-module direct data manipulation
...
```
tests/Feature/Modules/ActionItems/Api/ActionItemApiTest.php
tests/Unit/Modules/Risks/Services/RiskServiceTest.php
```
...
```
{timestamp}_create_{module}_{table}_table.php

Examples:
2026_01_25_000001_create_milestones_table.php
2026_01_25_000002_create_milestone_groups_table.php
2026_01_25_000003_create_risks_table.php
2026_01_25_000004_create_risk_groups_table.php
```
...
```php
// app/Modules/<Module>/Policies/<Entity>Policy.php
...
- Flat, intuitive module organization
